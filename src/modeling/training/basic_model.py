...
def evaluate(model, loader, criterion, vocab, device):
    ...
    for batch in loader:
        ...
        # Make preds as were notw in eval (Pres is most likely logit)
...
def save_training_plots(history: dict, save_dir: Path) -> None:
    ...
    # Character Accuracy
    axes[0, 1].plot(epochs, history["val_char_acc"], "o-", label="Val", color="orange")
    ...
    # Gradient Norm
    axes[1, 0].plot(epochs, history["grad_norm"], "o-", color="red")
    ...
    gap = [val_loss - train_loss for train_loss, val_loss in zip(history["train_loss"], history["val_loss"])]
    axes[1, 1].plot(epochs, gap, "o-", color="purple")
    ...
    fig.suptitle("Basic Model Training Metrics", fontsize=14)
    fig.tight_layout()
